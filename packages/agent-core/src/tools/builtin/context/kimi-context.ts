...
export const KIMI_CONTEXT_TOOL_NAME = 'KimiContext';

export const KimiContextInputSchema = z
  .object({
    query: z
      .string()
      .min(1)
...
export type KimiContextInput = z.Infer<typeof KimiContextInputSchema>;
...
export class KimiContextTool implements BuiltinTool<KimiContextInput> {
  readonly name = KIMI_CONTEXT_TOOL_NAME;
  readonly description = DESCRIPTION;
  readonly parameters: Record<string, unknown> = toInputJsonSchema(KimiContextInputSchema);
  ...
  resolveExecution(args: KimiContextInput): ToolExecution {
    const parsed = KimiContextInputSchema.safeParse(args);
    if (!parsed.success) {
