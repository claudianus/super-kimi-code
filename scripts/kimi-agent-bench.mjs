...
import { runBenchmark, runLoop } from './kimi-agent-bench/benchmark.mjs';
import { createRunId, parseArgs, usage } from './kimi-agent-bench/cli-options.mjs';
import { DEFAULT_EVIDENCE_BASE } from './kimi-agent-bench/constants.mjs';
import { appendJsonl, commandRecord, runBoundedCommand, writeJson } from './kimi-agent-bench/utils.mjs';
...
function validateReplayArgv(argv) {
  ...
  if (argv[0] !== 'node' || argv[1] !== 'scripts/kimi-agent-bench.mjs') {
    throw new Error('Replay rerun.argv must begin with node scripts/kimi-agent-bench.mjs.');
  }
