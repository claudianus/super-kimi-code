...
function runUltraworkSolver(commandLogPath, sourceCheckout, task, workspace, runner = 'ultrawork') {
  ...
  const brandedRunnerName = runner === 'provider' ? 'Super Kimi provider' : 'Ultrawork';
  if (missing.length > 0) {
    const record = {
      name: runner === 'provider' ? 'super-kimi-provider-solver-preflight' : 'ultrawork-solver-preflight',
      runner,
      providerRunner: 'super-kimi-ultrawork',
      taskId: task.id,
      status: 'BLOCKED',
  ...
  const kimiHome = path.join(workspace, '.kimi-home');
  const prompt = [
  ...
  const env = {
    ...
    KIMI_CODE_CACHE_DIR: path.join(workspace, '.kimi-cache'),
  };
  ...
  const record = commandRecord(runner === 'provider' ? 'super-kimi-provider-solver' : 'ultrawork-solver', task.id, result, {
    runner,
    providerRunner: 'super-kimi-ultrawork',
    requiredEnv: PROVIDER_REQUIRED_ENV,
    missingEnv: [],
