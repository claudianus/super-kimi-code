...
- Upstream `#1132` selective: filter `/thinking` slash autocomplete through active model effort metadata.
...
- Preserved the existing `/thinking` command shape while making its autocomplete suggest only valid efforts for the active model, including hiding `off` for always-thinking aliases.
