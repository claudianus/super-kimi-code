...
编辑配置或导入 provider 后，运行 `kimi provider doctor`。它会检查缺失的环境变量引用、格式错误的 per-credential `base_url`、重复 label、损坏的 fallback alias，以及无效的 preferred credential label，并且不会打印 API key 或 OAuth storage key。
