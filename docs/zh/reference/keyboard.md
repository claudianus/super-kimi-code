...
按 `Shift-Tab` 可开启或关闭 Ultrawork mode。开启后，下一个普通 prompt 会先进入 UltraPlan interview，再形成可验证的 UltraGoal，然后按 research、Swarm decision、integration、verification、learning 的顺序推进。除非 Ultrawork mode 已开启，或 prompt 明确要求 UltraWork，普通 prompt 不会进入这个工作流。
