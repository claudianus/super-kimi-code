...
> Windows 用户首次启动前还需要安装 [Git for Windows](https://gitforwindows.org/)，Super Kimi Code 会使用其中的 Git Bash 作为 Shell 环境。如果 Git Bash 安装在非标准路径，请把 `KIMI_SHELL_PATH` 设为 `bash.exe` 的绝对路径。

脚本会检出本 GitHub 源码仓库、构建 CLI，并把 `skimi` 可执行文件放到你的 `PATH` 中。需要 Git 和 Node.js 24.15.0 或更高版本。
...
默认会把仓库检出到 `~/.super-kimi-code/source`。
...
**卸载**：删除 `skimi` 可执行文件和 `~/.super-kimi-code/source` 源码检出目录。

## 第一次启动

进入项目目录后直接运行 `skimi` 启动交互界面：
...
::: tip 使用其他 AI 供应商
如果你想接入 Anthropic、OpenAI、Google 等其他供应商，可以在 TUI 中打开 `/provider`，也可以在 shell 中使用 `kimi provider catalog add` / `kimi provider custom add`。对于有多个账号或 API key 的团队，`kimi provider key add`、`kimi provider oauth add` 和 `kimi provider route auto` 可以创建 quota-aware fallback route，且不会暴露 secret。详见[平台与模型](../configuration/providers.md)。
:::
