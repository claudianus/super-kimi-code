...
export async function handleComputerUseCommand(
    ...
    if (!status.installed) {
      ...
      if (!status.installed && !result.ok) {
        resolved.stderr.write(
          'Computer-use auto-install failed. Retry with `kimi computer-use install`.\n',
        );
      }
    }
  ...
  resolved.stderr.write(`Computer-use ${action} failed. Retry with \`kimi computer-use ${action}\`.\n`);
  return 1;
}
...
function permissionGuidance(platform: NodeJS.Platform): string {
  if (platform === 'darwin') {
    return [
      ...
      '- Grant Accessibility to the terminal or app that launches Kimi Code.',
      '- Grant Screen Recording to the same launcher, and to CuaDriver.app if macOS prompts for it.',
      '- Re-run `kimi computer-use doctor` after changing permissions.',
      '',
    ].join('\n');
  }
  if (platform === 'win32') {
    return [
      ...
      '- Apps running elevated may require launching Kimi Code from an elevated terminal.',
      '- Re-run `kimi computer-use doctor` after changing permissions.',
      '',
    ].join('\n');
  }
