...
import type { ComputerUseRuntime, SetupCommandResult } from '@moonshot-ai/gui-use';
...
describe('browser-use CLI commands', () => {
  it('runs CloakBrowser install from the host package root', async () => {
    ...
    await expect(handleBrowserUseCommand('install', {
      ...writable,
      packageRoot: () => '/repo/apps/kimi-code',
      install,
    })).resolves.toBe(0);

    expect(install).toHaveBeenCalledWith({ cwd: '/repo/apps/kimi-code', quiet: true });
  ...
  it('reports recovery command when CloakBrowser status fails', async () => {
    ...
    expect(stderr.join('')).toContain('kimi browser-use install');
  });
});
