...
const installScript = resolve(repoRoot, 'scripts/install-skimi.mjs');
...
async function makeHome(): Promise<string> {
  const home = await mkdtemp(join(tmpdir(), 'skimi-install-home-'));
  tempHomes.push(home);
  return home;
}
...
describe('scripts/install-skimi.mjs', () => {
  afterEach(async () => {
    await Promise.all(tempHomes.splice(0).map((home) => rm(home, { recursive: true, force: true })));
  });

  it.skipIf(process.platform === 'win32')('installs skimi and updates bash, zsh, and fish startup files idempotently', async () => {
    const home = await makeHome();

    await runInstall(home, ['--bin-dir', '~/local-bin']);
    await runInstall(home, ['--bin-dir', '~/local-bin']);

    const wrapper = await readFile(join(home, 'local-bin/skimi'), 'utf-8');
    expect(wrapper).toContain('Managed by super-kimi-code scripts/install-skimi.mjs');
    ...
    for (const file of ['.zshrc', '.bashrc', '.profile']) {
      ...
      expect(text.match(/# >>> skimi PATH >>>/g)).toHaveLength(1);
      expect(text).toContain('skimi_bin_dir="$HOME/local-bin"');
      expect(text).toContain('export PATH="$skimi_bin_dir:$PATH"');
    }

    const fish = await readFile(join(home, '.config/fish/config.fish'), 'utf-8');
    expect(fish.match(/# >>> skimi PATH >>>/g)).toHaveLength(1);
    expect(fish).toContain('set -l skimi_bin_dir "$HOME/local-bin"');
    expect(fish).toContain('fish_add_path $skimi_bin_dir');
  });

  it.skipIf(process.platform === 'win32')('does not replace an unmanaged command unless forced', async () => {
    const home = await makeHome();
    await writeFile(join(home, 'skimi'), 'user file\n');
    ...
    const wrapper = await readFile(join(home, 'skimi'), 'utf-8');
    expect(wrapper).toContain('Managed by super-kimi-code scripts/install-skimi.mjs');
  });

  it('keeps source install entrypoints on the Super Kimi GitHub repository', async () => {
    const sh = await readFile(posixSourceInstallScript, 'utf-8');
    const ps1 = await readFile(windowsSourceInstallScript, 'utf-8');

    expect(sh).toContain('https://github.com/claudianus/super-kimi-code.git');
    expect(ps1).toContain('https://github.com/claudianus/super-kimi-code.git');
    expect(sh).not.toContain('code.kimi.com/kimi-code');
    expect(ps1).not.toContain('code.kimi.com/kimi-code');
    expect(sh).toContain('scripts/install-skimi.mjs');
